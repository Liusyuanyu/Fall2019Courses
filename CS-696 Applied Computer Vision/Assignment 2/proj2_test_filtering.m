...
test_image = im2single(imread('../data/submarine.bmp'));
...
% identity_image  = my_filter(test_image, identity_filter);
...
figure('name', 'imfilter'); imshow(blur_image);

% imwrite(blur_image, 'blur_image.jpg', 'quality', 95);

blur_image = my_imfilter(test_image, blur_filter);
figure('name', 'my_imfilter'); imshow(blur_image);
...
% large_blur_image = my_filter(test_image, large_1d_blur_filter);
% large_blur_image = my_filter(large_blur_image, large_1d_blur_filter'); %notice the ' operator which transposes the filter

figure(4); imshow(large_blur_image);


large_blur_image_ori = imfilter(test_image, large_1d_blur_filter);
large_blur_image_ori = imfilter(large_blur_image_ori, large_1d_blur_filter'); %notice the ' operator which transposes the filter
figure(5); imshow(large_blur_image_ori);
...
% sobel_image = my_filter(test_image, sobel_filter);

%0.5 added because the output image is centered around zero otherwise and mostly black
figure(5); imshow(sobel_image);
% figure(5); imshow(sobel_image + 0.5);
% imwrite(sobel_image + 0.5, 'sobel_image.jpg', 'quality', 95);

sobel_image_ori = imfilter(test_image, sobel_filter);
figure(6); imshow(sobel_image_ori);
...
% laplacian_image = my_filter(test_image, laplacian_filter);
...
laplacian_image_ori = imfilter(test_image, laplacian_filter);
% figure(7); imshow(laplacian_image_ori + 0.5);
imwrite(laplacian_image_ori + 0.5, 'laplacian_image_ori.jpg', 'quality', 95);
